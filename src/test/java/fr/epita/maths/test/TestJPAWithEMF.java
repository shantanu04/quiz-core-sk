...
public class TestJPAWithEMF {
	...
	public void test() {
		...
		// then
		TypedQuery<Question> searchQuery = em.createQuery("from Question as q where q.questionLabel = :qlabel", Question.class);
		searchQuery.setParameter("qlabel", question.getQuestionLabel());
		
		Assert.assertNotEquals(0, searchQuery.getResultList().size());

		TypedQuery<MCQChoice> searchQueryMCQ = em.createQuery("from MCQChoice as m where m.question = :question", MCQChoice.class);
